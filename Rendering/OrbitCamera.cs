...
public sealed class OrbitCamera
{
    ...
    public const float MaxTheta  = PI - 0.01f;
    ...
    {
        Phi   += dX * sensitivity;                                  
        Theta  = float.Clamp(Theta + dY * sensitivity, MinTheta, MaxTheta);
    }

    public void Zoom(float delta, float factor = 0.15f)
    {
        Radius = float.Clamp(Radius * (1f - delta * factor), MinRadius, MaxRadius);
    }
